  ...
  return (
    <SpotlightCard className="flex flex-col items-center gap-x-8 gap-y-4 p-4 sm:flex-row">
      <div className="flex flex-col gap-y-2">
        <div className="flex gap-x-4">
          ...
          <div className="flex flex-col">
            <span className="text-2xl font-medium text-green-600">{data?.name}</span>
            <Tooltip title={`${durationDays} ${t('days_ago')}`}>
              <span className="text-xs text-neutral-600 dark:text-neutral-400">
                {t('joined')} {format(date, 'dd MMM yyyy')}
              </span>
            </Tooltip>
            <Tooltip title={`${t('current_streak')}: ${data?.maxStreak} ${t('unit_days')}`}>
              <span className="text-xs text-neutral-600 dark:text-neutral-400">
      ...
      <div className="flex flex-grow flex-col items-center justify-between rounded-xl sm:flex-row sm:px-4 sm:py-3">
        <Item label={t('title_test_started')} value={data?.typingStats.startedTests || 'N/A'} />
        <Item label={t('title_test_completed')} value={data?.typingStats.completedTests || 'N/A'} />
        <Item
          label={t('title_time_typing')}
          value={format(new Date(0, 0, 0, 0, minutes, seconds), 'HH:mm:ss') || 'N/A'}
        />
      </div>
    </SpotlightCard>
  );
